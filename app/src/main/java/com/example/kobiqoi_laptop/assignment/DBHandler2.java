package com.example.kobiqoi_laptop.assignment;

import android.content.ContentValues;
import android.content.Context;
import android.database.Cursor;
import android.database.sqlite.SQLiteDatabase;
import android.database.sqlite.SQLiteOpenHelper;

import java.util.ArrayList;
import java.util.List;

public class DBHandler2 extends SQLiteOpenHelper {
    private static final int DATABASE_VERSION = 1;
    private static final String DATABASE_NAME = "tablesManager";
    private static final String TABLE_TABLES = "tables";
    private static final String KEY_ID = "id";
    private static final String KEY_TABLENUMBER = "table_number";
    private static final String KEY_TABLESIZE = "table_size";
    //private static final String KEY_EMAIL = "email";
    //private static final String KEY_PASSWORD = "password";
    public DBHandler2(Context context) {
        super(context, DATABASE_NAME, null, DATABASE_VERSION);
    }
    // Creating Table
    @Override
    public void onCreate(SQLiteDatabase db) {
        String CREATE_TABLES_TABLE = "CREATE TABLE " + TABLE_TABLES + "("
                + KEY_ID + " INTEGER PRIMARY KEY," + KEY_TABLENUMBER + " TEXT,"
                + KEY_TABLESIZE + " TEXT" + ")";
        db.execSQL(CREATE_TABLES_TABLE);
    }
    // Upgrading database
    @Override
    public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
        // Drop older table if existed
        db.execSQL("DROP TABLE IF EXISTS " + TABLE_TABLES);
        // Create table again
        onCreate(db);
    }
    // Adding new student
    void addTable(Table table) {
        SQLiteDatabase db = this.getWritableDatabase();
        ContentValues values = new ContentValues();
        values.put(KEY_TABLENUMBER, table.getTablenumber()); // table number
        values.put(KEY_TABLESIZE, table.getTablesize()); // table size
        //values.put(KEY_EMAIL, account.getEmail()); // Contact Email
        //values.put(KEY_PASSWORD, account.getPassword()); // Contact Password
        // Inserting Row
        db.insert(TABLE_TABLES, null, values);
        //2nd argument is String containing nullColumnHack
        db.close(); // Closing database connection
    }
    // Getting single account
    Table getTable(int id) {
        SQLiteDatabase db = this.getReadableDatabase();
        Cursor cursor = db.query(TABLE_TABLES, new String[] { KEY_ID,
                        KEY_TABLENUMBER, KEY_TABLESIZE }, KEY_ID + "=?",
                new String[] { String.valueOf(id) }, null, null, null, null);
        if (cursor != null)
            cursor.moveToFirst();
        Table table = new Table(Integer.parseInt(cursor.getString(0)),
                cursor.getString(1), cursor.getString(2));
        // return account
        return table;
    }
    // Getting All Contacts
    public ArrayList<Table> getAllTables() {
        ArrayList<Table> tableList = new ArrayList<Table>();
        // Select All Query
        String selectQuery = "SELECT * FROM " + TABLE_TABLES;
        SQLiteDatabase db = this.getWritableDatabase();
        Cursor cursor = db.rawQuery(selectQuery, null);
        // looping through all rows and adding to list
        if (cursor.moveToFirst()) {
            do {
                Table table = new Table();
                table.setID(Integer.parseInt(cursor.getString(0)));
                table.setTablenumber(cursor.getString(1));
                table.setTablesize(cursor.getString(2));
                //account.setEmail(cursor.getString(3));
                //account.setPassword(cursor.getString(4));
                // Adding student to list
                tableList.add(table);
            } while (cursor.moveToNext());
        }
        // return student list
        return tableList;
    }
    // Updating single student
    public int updateTable(Table table) {
        SQLiteDatabase db = this.getWritableDatabase();
        ContentValues values = new ContentValues();
        values.put(KEY_TABLENUMBER, table.getTablenumber());
        values.put(KEY_TABLESIZE, table.getTablesize());
        //values.put(KEY_EMAIL, account.getEmail());
        //values.put(KEY_PASSWORD, account.getPassword());
        // updating row
        return db.update(TABLE_TABLES, values, KEY_ID + " = ?",
                new String[] { String.valueOf(table.getID()) });
    }
    // Deleting single student
    public void deleteTable(Table table) {
        SQLiteDatabase db = this.getWritableDatabase();
        db.delete(TABLE_TABLES, KEY_ID + " = ?",
                new String[] { String.valueOf(table.getID()) });
        db.close();
    }
    // Getting student Count
    public int getTablesCount() {
        String countQuery = "SELECT * FROM " + TABLE_TABLES;
        SQLiteDatabase db = this.getReadableDatabase();
        Cursor cursor = db.rawQuery(countQuery, null);
        cursor.close();
        // return count
        return cursor.getCount();
    }
}
